...
const firebaseConfig = {
    apiKey: "AIzaSyAfHOSzWif25YzwDTu14SoTWU6mZ2TOZ70",
    authDomain: "janpath-sadhna.firebaseapp.com",
    projectId: "janpath-sadhna",
    storageBucket: "janpath-sadhna.firebasestorage.app",
    messagingSenderId: "625944659407",
    appId: "1:625944659407:web:c798a73f5961e687f67eba"
};
...
const visibleCategories = () => {
    if (isSuperAdmin()) return ['Coordinators','Overall Coordinators'];
    if (isCategoryAdmin()) {
...
function nrPenalty(level) { return -30; }
...
function getNRData(date, level) {
    ...
    return {
        ...
        scores:{ sleep:-5, wakeup:-5, chanting:-5, reading:-5, hearing:-5, service:-5, notes:0, daySleep:0 }
    };
}
...
function calculateScores(slp, wak, chn, rMin, hMin, sMin, nMin, dsMin, level) {
    const sc = { sleep:-5, wakeup:-5, chanting:-5, reading:-5, hearing:-5, service:-5, notes:0, daySleep:0 };
    ...
    sc.reading  = act(rMin, 30);
    sc.hearing  = act(hMin, 30);
    sc.service  = act(sMin, 30);
    sc.notes    = 0;
    let total = sc.sleep + sc.wakeup + sc.chanting + sc.reading + sc.hearing + sc.service + sc.daySleep;
...
window.downloadUserExcel = async (userId, userName) => {
    ...
    try {
        ...
        sortedWeeks.forEach((sunStr, wi) => {
            ...
            const actMaxMap = { sl:25, wu:25, ch:25, rd:25, hr:25, sv:25, nt:0, ds:10 };
...
window.downloadMasterReport = async () => {
    ...
    try {
        ...
        userData.forEach(({user,entries}) => {
            const row = [user.name, user.level||'Coordinators', user.chantingCategory||'Level-1'];
...
window.downloadCategoryExcel = async (category) => {
    if (typeof XLSX === 'undefined') { alert('Excel library not loaded. Please refresh.'); return; }
    const catLabel = category.replace('Overall Coordinators','OC').replace('Coordinators','Coord');
    try {
        const usersSnap = await db.collection('users').get();
            ...
            sortedWeeks.forEach(wLabel => {
                ...
                for (let i = 0; i < 7; i++) {
                    ...
                    if (e) {
                        ...
                        tot += _nrP;
                        rows.push([lbl, 'NR',-5,'NR',-5,'NR',-5,0,-5,0,-5,0,-5,0,0,0,0,_nrP,Math.round((_nrP/160)*100)+'%']);
                    }
                }
...
function initDashboard() {
    const roleLabel = isSuperAdmin() ? '👑 Super Admin'
                    ...
                    : (userProfile.level || 'Coordinators');
...
async function loadUserWCR() {
    ...
    filtered.forEach((uDoc, rowIdx) => {
        ...
        const lvlShort = (u.level||'Coordinators').replace('Overall Coordinators','OC').replace('Coordinators','Coord');
...
async function loadHomePanel(weekOffset) {
    ...
    const actMax    = { sleep: 25, wakeup: 25, chanting: 25, reading: 25, hearing: 25, service: 25, notes: 0 };

    const uJd = userProfile?.joinedDate || APP_START;
    const uLevel = userProfile?.level || 'Coordinators';
...
async function loadSAHome(weekOffset, btn) {
    ...
    const usersSnap = await db.collection('users').get();
    const allCats = ['Coordinators','Overall Coordinators'];
    const catCounts = {};
    ...
    nonSADocs.forEach((uDoc, idx) => {
        const u = uDoc.data();
        const lvl = u.level || 'Coordinators';
        ...
        const lvlShort = lvl.replace('Overall Coordinators','OC').replace('Coordinators','Coord');
    ...
    const catLabels = { 'Coordinators': 'Coord', 'Overall Coordinators': 'OC' };
    allCats.forEach(c => {
...
window.openUAC = (uid, name, level, chanting, rounds, role) => {
    ...
    if (isSuperAdmin()) {
        ...
        if (role === 'superAdmin') {
            ...
            opts += '<option value="superAdmin">👑 Make Super Admin</option><option disabled>── Shift Category ──</option><option value="shift:Coordinators">⭐ Shift to Coordinators</option><option value="shift:Overall Coordinators">⭐ Shift to Overall Coordinators</option><option disabled>── Make Admin ──</option><option value="cat:Coordinators">🛡️ Admin — Coordinators</option><option value="cat:Overall Coordinators">🛡️ Admin — Overall Coordinators</option><option value="demote">🚫 Revoke Admin</option>';
        } else {
            opts += '<option value="superAdmin">👑 Make Super Admin</option><option disabled>── Shift Category ──</option><option value="shift:Coordinators">⭐ Shift to Coordinators</option><option value="shift:Overall Coordinators">⭐ Shift to Overall Coordinators</option><option disabled>── Make Admin ──</option><option value="cat:Coordinators">🛡️ Admin — Coordinators</option><option value="cat:Overall Coordinators">🛡️ Admin — Overall Coordinators</option>';
        }
...
async function fetchChartData(userId, view) {
    ...
    const _chartLevel = uDoc.exists ? (uDoc.data().level || 'Coordinators') : 'Coordinators';
    const allEntries = [];
    snap.forEach(doc => {
...
document.getElementById('sadhana-form').onsubmit = async (e) => {
    ...
    const level = userProfile.level || 'Coordinators';
            ...
            return `<div class="inactive-card">
                <div class="inactive-card-left">
                    ...
                    <div>
                        ...
                        <div class="inactive-meta">${esc(u.level||'Coordinators')} · ${lastTxt} · <strong>${u.missedDays} days missed</strong></div>
                    </div>
                </div>
...
window.openEditModal = async (userId, date) => {
    ...
    const uLevel  = uSnap.exists ? (uSnap.data().level || 'Coordinators') : 'Coordinators';
    ...
    document.getElementById('edit-notes-row').classList.add('hidden');
    updateEditPreview();
...
window.updateEditPreview = () => {
    ...
    const level = document.getElementById('edit-user-level').value || 'Coordinators';
...
function renderPerformersFromCache() {
    ...
    if (_activeFilter) {
        data = data.filter(u => {
            const lvl = u.level;
            if (_activeFilter === 'Coord') return lvl === 'Coordinators';
            if (_activeFilter === 'OC')    return lvl === 'Overall Coordinators';
            return true;
        });
    }
...
window.loadLeaderboard = async (force) => {
    ...
    try {
        ...
        let levelFilter;
        if (isSuperAdmin()) {
            levelFilter = _lbCategoryFilter ? [_lbCategoryFilter] : ['Coordinators','Overall Coordinators'];
        ...
        if (!isWeekly) {
            ...
            filtered.forEach((uDoc, i) => {
                ...
                rows.push({
                    ...
                    level:    (u.level || '').replace('Overall Coordinators','OC').replace('Coordinators','Coord'),
        ...
        filtered.forEach(uDoc => {
            const u = uDoc.data();
            const lvl = (u.level||'').replace('Overall Coordinators','OC').replace('Coordinators','Coord');
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:12px;">
            ...
            <button class="sa-lvl-btn lb-filter-btn${_lbCategoryFilter==='Coordinators'?' active':''}" onclick="setLbFilter('Coordinators',this)">Coordinators</button>
            <button class="sa-lvl-btn lb-filter-btn${_lbCategoryFilter==='Overall Coordinators'?' active':''}" onclick="setLbFilter('Overall Coordinators',this)">Overall Coordinators</button>
        </div>` : '';
...
async function renderActivityAnalysis(uid, period) {
    ...
    try {
        ...
        const actKeys   = ['sleep','wakeup','chanting','reading','hearing','service','daySleep'];
